...
== The Script
 ...
 TUN_IF=tun11                     # Interface name of the tunnel interface.
...
These are additional options that you can add to the script above if a standalone OpenVPN service is being used. This needs to be inserted before "DHCP Server" section. 
 ...
 ############## Tunnel Module ##############
 if [ `lsmod | grep -c tun` == 0 ]; then    # Change tun to tap if you are connecting using a tap device.
 	insmod tun
 	sleep 1
 fi
...
The following is to setup a cron job. This simply reruns this script at whatever times specified to ensure that everything is still setup. Place this at the end of the file. 
