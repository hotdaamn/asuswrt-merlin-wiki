...
The first part of this script is where pertinent information is put in in order to setup the extra network and create the additional routes, rules, and settings. The second part (the part after the two lines of #) is where the commands are executed to set everything up. Nothing should have to be changed. 

=== Part 1


==Implementing
1. Create a file in the configs folder of jffs and name it what you like (e.g. vpn_ssid) Copy the above script into this file and save.
Next, place this line
 /jffs/configs/your_file_name (e.g. vpn_ssid)
in the services-start file in the scripts folder.

2. Open up a terminal to the router or go to the Run Cmd tab in the web interface and type the following command.

 chmod 0755 /jffs/config/your_file_name
...
The following is to setup a cron job. This simply reruns this script at whatever times specified to ensure that everything is still setup.
